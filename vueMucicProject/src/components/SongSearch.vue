<template>
  <li class="songitem" @click="$emit('tanslate-song',songItemDispose())">
    ...
    <div class="main" :style="{marginLeft:options.order!==undefined?'0px':'10px'}">
      <h3>
        ...
        <span class="alias">{{songItem.alias[0]}}</span>
      </h3>
...
export default {
  ...
  methods: {
    ...
    songItemDispose(){
      // 获取通过歌曲详细获取图片
        return{
              id:this.songItem.id,
              picUrl:this.getSongImg,
              name:this.songItem.name,
              ar:this.songItem.artists,
              dt:this.songItem.duration
            };
    }
  ...
  created(){
        ...
        .get("/song/detail", {
          params: {
           ids:this.songItem.id
          }
        })
        .then(response => {
          this.getSongImg=response.data.songs[0].al.picUrl;
        })
        .catch(error => {
          console.log(error);
        });
